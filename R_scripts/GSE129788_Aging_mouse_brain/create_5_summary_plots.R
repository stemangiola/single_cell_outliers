...
sum_table <- readRDS("/stornext/Bioinf/data/bioinf-data/Papenfuss_lab/projects/ma.m/single_cell_outliers/single_cell_database/GSE129788_aging_mouse_brain/data/final_sum_table/merged_sum_table.rds")
...
bar_plot = sum_table %>% ggplot(aes(x = variable, y = Count,fill = variable)) +
    ...
    ggtitle(glue("summary table of dataset (GSE129788) for method: {method}"))
...
ppcseq_dir = "/stornext/Bioinf/data/bioinf-data/Papenfuss_lab/projects/ma.m/single_cell_outliers/single_cell_database/GSE129788_aging_mouse_brain/data/ppcseq_data/"
...
histogram_plot <- ppcseq_df_FC %>% ggplot(aes(x = FC)) +
    ...
    ggtitle(glue("Histogram of fold changes of differential transcripts with and without outliers for for dataset (GSE129788) with method: {method}")) +
