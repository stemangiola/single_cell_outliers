...
in_dir = "/stornext/Bioinf/data/bioinf-data/Papenfuss_lab/projects/ma.m/single_cell_outliers/single_cell_database/SCP1288_renal_cell_carcinoma/data/de_data/"
...
plot_dir = "/stornext/Bioinf/data/bioinf-data/Papenfuss_lab/projects/ma.m/single_cell_outliers/single_cell_database/SCP1288_renal_cell_carcinoma/plot/"
...
dev.new()
density_plot <- df %>% 
    ggplot(aes(x = abundance_scaled + 1, color = sample)) +
    geom_density() +
    facet_wrap(~ cell_type) +
    scale_x_log10() +
    ggtitle("Density plot for 33 cell types of dataset (SCP1288)") +
    custom_theme 
...
PCA_plot <- df_for_PCA %>% 
    ...
    geom_point() +
    facet_wrap(~ cell_type) +
    ggtitle(glue("PCA plot of dataset (SCP1288) with method: {method}")) +
    custom_theme
...
# bar plot
df <- df %>% 
    mutate(abundance_RNA = abundance %>% as.integer) %>% 
...
sum_table <- readRDS("/stornext/Bioinf/data/bioinf-data/Papenfuss_lab/projects/ma.m/single_cell_outliers/single_cell_database/SCP1288_renal_cell_carcinoma/data/final_sum_table/merged_sum_table.rds")
...
bar_plot = sum_table %>% ggplot(aes(x = variable, y = Count,fill = variable)) +
    ...
    ggtitle(glue("summary table of dataset (SCP1288) for method: {method}"))
...
ppcseq_dir = "/stornext/Bioinf/data/bioinf-data/Papenfuss_lab/projects/ma.m/single_cell_outliers/single_cell_database/SCP1288_renal_cell_carcinoma/data/ppcseq_data/"
...
ppcseq_df = ppcseq_df %>% unnest(cols = file_contents) %>% 
    select(filename, transcript, tot_deleterious_outliers, method) %>% 
...
volcano_plot <- data_for_volanco %>%
    ...
    # ggplot(aes(x = !!sym(logFC_column_name), y = !!sym(pvalue_column_name))) +
...
histogram_plot <- ppcseq_df_FC %>% ggplot(aes(x = FC)) +
    ...
    ggtitle(glue("Histogram of fold changes of differential transcripts with and without outliers for for dataset (SCP1288) with method: {method}")) +
