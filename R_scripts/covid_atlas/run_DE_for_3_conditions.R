...
counts = counts %>%
    ...
    tidybulk::test_differential_abundance(
        ...
        .abundance = abundance_RNA,
        contrasts = c(
            "(1/2 * disease_severity_standardmoderate + 1/2 * disease_severity_standardsevere) - disease_severity_standardhealthy",
            "disease_severity_standardsevere - (1/2 * disease_severity_standardmoderate + 1/2 * disease_severity_standardhealthy)"
        )
    )  %>%
    
    mutate(abundance_RNA = abundance_RNA %>% as.integer()) %>%
    mutate(is_significant = `FDR___(1/2 * disease_severity_standardmoderate + 1/2 * disease_severity_standardsevere) - disease_severity_standardhealthy` < 0.05 |
               `FDR___disease_severity_standardsevere - (1/2 * disease_severity_standardmoderate + 1/2 * disease_severity_standardhealthy)` < 0.05) %>% 
    
    mutate(.significance = (`PValue___(1/2 * disease_severity_standardmoderate + 1/2 * disease_severity_standardsevere) - disease_severity_standardhealthy` +
                                `PValue___disease_severity_standardsevere - (1/2 * disease_severity_standardmoderate + 1/2 * disease_severity_standardhealthy)`) / 2  
               ) %>% 
