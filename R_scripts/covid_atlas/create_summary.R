...
if (nrow(ppcseq_data) != 0) {
    ...
    outlier_summary_table <-
        tibble(cell_type = cell_name,
               total_genes = de_data$transcript %>% n_distinct(),
               num_de_genes = nrow(outlier_data),
               num_de_genes_with_outliers = sum(outlier_data$tot_deleterious_outliers > 0),
               num_de_genes_with_outliers_in_top_10_PValue = sum(
    ...
    my_tibble = tibble(cell_type = cell_name,
                       total_genes = de_data$transcript %>% n_distinct(),
                       num_de_genes = as.integer(0),
                       num_de_genes_with_outliers = as.integer(0),
                       num_de_genes_with_outliers_in_top_10_PValue = as.integer(0)) %>% 
