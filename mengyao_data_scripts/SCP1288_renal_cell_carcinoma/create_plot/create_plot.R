...
in_dir = "/stornext/HPCScratch/home/ma.m/single_cell_database/SCP1288_renal_cell_carcinoma/data/de_data/" # path to data
...
density_plot <- df %>% 
    ...
    scale_x_log10() +
    ggtitle("Density plot for 11 cell types of dataset (SCP1288)") +
    custom_theme 
...
PCA_plot <- df_for_PCA %>% 
    ...
    facet_wrap(~ cell_type) +
    ggtitle("PCA plot for 11 cell types for dataset (SCP1288)") +
    custom_theme
...
sum_table <- readRDS("/stornext/HPCScratch/home/ma.m/single_cell_database/SCP1288_renal_cell_carcinoma/data/final_sum_table/sum_table.rds")
...
sum_table <- sum_table %>% pivot_longer(cols = c(transcript, de_genes, genes_with_outliers, outliers_in_top_PValue),
                                        names_to = "variable",
                                        values_to = "Count")
sum_table %>% saveRDS("/stornext/HPCScratch/home/ma.m/single_cell_database/SCP1288_renal_cell_carcinoma/data/final_sum_table/sum_table_for_bar_plot.rds")
# draw bar plot
...
bar_plot = sum_table %>% ggplot(aes(x = variable, y = Count,fill = variable)) +
    ...
    ggtitle("summary table of dataset (SCP1288)")
...
data_for_volanco %>% saveRDS("/stornext/HPCScratch/home/ma.m/single_cell_database/SCP1288_renal_cell_carcinoma/data/final_sum_table/dataframe_for_volcano.rds")
